...
def topics(request):
    """Show all topics."""
    users_topics = (Topic.objects
                    .filter(owner=request.user)
                    .order_by('date_added'))
    context = {'topics': users_topics}
...
def new_entry(request, topic_id):
    ...
    if request.method != 'POST':
        ...
        if form.is_valid():
            ...
            created_entry.topic = selected_topic
            created_entry.save()
            return redirect('learning_logs:topic', topic_id=topic_id)

    # Display a blank or invalid form.
    context = {'topic': selected_topic, 'form': form}
...
def edit_entry(request, entry_id):
    ...
    if request.method != 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('learning_logs:topic', topic_id=selected_topic.id)

    context = {'entry': entry, 'topic': selected_topic, 'form': form}
